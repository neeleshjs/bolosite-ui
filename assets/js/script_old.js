(() => {
  ...
  const updateBackToTopFab = () => {
    if (!backToTopFab) return;
    backToTopFab.classList.toggle('visible', window.scrollY > 520);
  };
  updateBackToTopFab();
  window.addEventListener('scroll', updateBackToTopFab, { passive: true });
  ...
  const initSectionLogoAssemblies = () => {
    ...
    sections.forEach((section, sectionIndex) => {
      ...
      section.classList.add('bs-logo-zone');
    ...
    const setInitialLogoMotion = section => {
      const rect = section.getBoundingClientRect();
      const buffer = Math.max(280, window.innerHeight * 0.5);
      section.classList.toggle('bs-logo-live', rect.bottom >= -buffer && rect.top <= window.innerHeight + buffer);
    };
    ...
    if ('IntersectionObserver' in window) {
      ...
      sections.forEach(section => {
        setInitialLogoMotion(section);
        logoMotionObserver.observe(section);
      });
    } else {
  ...
  document.getElementById('logoutButton').addEventListener('click', async () => {
    try { await apiRequest('/api/public/logout', { method: 'POST', body: '{}' }); } catch (_) {}
    document.getElementById('profileMenu').classList.remove('open');
    renderAccount(null);
  ...
  const quickDemoResult = question => {
    ...
    if (/feature|language|hindi|english|action|voice/.test(value)) return {
      ...
      label: 'How BoloSite helps', href: '#product'
    };
    return {
  ...
  quickDemoForm?.addEventListener('submit', event => {
    event.preventDefault();
  ...
  const updateEstimate = () => {
    const requests = Number(document.getElementById('monthlyRequests').value);
  ...
  ['monthlyRequests'].forEach(id => document.getElementById(id).addEventListener('input', updateEstimate));
  updateEstimate();
